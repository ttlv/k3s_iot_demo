...
func run(c *cli.Context) {
	...
	mq := k3s_iot_demo.Mqtt{
		Server: c.String("mqtt_server"),
		Topic:  c.String("mqtt_topic"),
	}
	cli, err := k3s_iot_demo.ConnectToMQTT("influxDB_client", mq)
	if err != nil {
		logrus.Fatal(err)
	}
...
func subscribeToMQTT(cli *mqClient.Client, mq k3s_iot_demo.Mqtt, conn *influxClient.Client) error {
		...
		SubReqs: []*mqClient.SubReq{
			&mqClient.SubReq{
				...
				Handler: func(topicName, message []byte) {
					...
					messageData := k3s_iot_demo.Message{}
...
func writeToDB(message k3s_iot_demo.Message, conn *influxClient.Client) {
